...
backToMenuButton.addEventListener('click', () => {
    ...
    // Ocultamos el contenedor del juego y mostramos el inicio sin animaciones
...
function getWordByEnemyType(type) {
    const wordLists = {
        ...
        moons: [
            'sol','luz','paz','casa','gato','mesa','vino','mano','foco','rayo',
            'miel','coro','rio','lima','saco','rana','beso','kilo','mono','oso',
            'liso','azul','vela','oro','ojo','olas','solo','pato','dato','lobo',
            'lata','fino','silo','pila','cupo','sopa','fuga','bajo','cima','toro',
            'seda','mar','pan','te','dos','risa','alma','muro','caja','nube'
        ],
        gas_giants: [
            // 2-4 letras con tilde (25)
            'sí','sé','té','día','tío','pío','mío','ríe','rió','vía',
            'fía','fió','aún','café','sofá','maíz','raíz','oía','oír','dúo',
            'veía','leía','guió','púa','íra','huía',
        
            // 5-6 letras sin tildes (25)
            'mundo','planta','viento','campo','largo','listo','pared','luces','curva','barco',
            'salsa','metro','grupo','tecla','ritmo','sudar','crear','golfo','sabio','playa',
            'bravo','rueda','golpe','frase','suave'
        ],
        stars: [
            'fácil','sólido','música','rápido','túnel','hábil','cálido','húmedo','médico','típico',
            'lógico','fétido','gótico','ánimo','cúpula','dórico','íntimo','tímido','mágico','cóndor',
            'cónico','pánico','cédula','céfiro','jónico','lámina','lívido','túnica','símil','várice',
            'láser','ámbar','tríada','ápice','géiser','ídolo','tóner','cártel','bastón','sandía',
            'módulo','débil','mítico','lácteo','órbita','támara','época','ético','ávido','íbero'
        ],
        galaxys: [
            'cósmica','metálico','océanos','retórico','artífice','síntesis','cráteres','político','cálidos','góndola',
            'efímero','tácitos','ráfagas','válidos','atlántis','rústicos','aérenos','práctico','códices','armónico',
            'métrico','génesis','apótema','polémica','católico','esférico','célebres','órbitas','árboles','carácter',
            'símbolos','mística','tórrido','vínculo','láminas','dédalos','jónicas','lánzalo','génicos','cóleras', 
            'cálices','vértigo','rótulos','prófugo','lívidos','cáscara','pólvora','túnicas','cínicos'
        ],
        black_holes: [
            'espaguetización','electrodoméstico','constitucional','internacional','hipersensibilidad',
            'electrocardiograma','computacional','cinematográfico','microorganismo','fotovoltaico',
            'desindustrialización','incomprensible','extraordinario','desproporcionado','multiplicándose',
            'sobrealimentación','contrarreloj','prerregistrando','infraestructuras','hiperconectividad',
            'electroquímico','aerotransportado','semifinalización','polinizadores','desmitificando',
            'ultramicroscópico','interdisciplinar','hipoalergénico','sobreestimando','automovilístico',
            'reorganización','caracterización','disponibilidades','parlamentarismo','antropomórfico',
            'infranqueable','electrostático','relacionalmente','impresionismo','retroalimentar',
            'desciframiento','meteorológico','especialización','configuración','objetivándose',
            'contemporáneo','desalinizadora','territorialidad','electroimán','revinculándose'
        ]
    };
...
function handleKeydown(e) {
    if ((e.ctrlKey || e.metaKey) && (e.key === '+' || e.key === '-' || e.key === '0')) {
        return; // No prevenir el comportamiento predeterminado
    }
...
function handleVirtualKeyPress(key) {
    ...
    // Si no hay enemigo actual, intentar encontrar uno que empiece con la tecla presionada
    if (!currentEnemy) {
    ...
    if (key === expectedChar) {
        ...
        if (isFinalLaser) {
            ...
            currentEnemy = null;
            // Aquí retornamos para evitar que, en el mismo "keyup",
            // se busque un nuevo enemigo con la misma letra.
            return; 
        }
    } else {
        // Aquí puedes agregar lógica si el jugador se equivoca, si lo deseas.
    }
}
